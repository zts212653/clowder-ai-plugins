...
interface ExecFileResult { readonly stdout: string; readonly stderr: string }
type ExecFileRunner = (
  executable: string,
  args: readonly string[],
  options: Readonly<Record<string, unknown>>,
) => Promise<ExecFileResult>;
const execFileAsync = promisify(execFile) as unknown as ExecFileRunner;
...
export class WeComCliExecutor {
  ...
  constructor(
    ...
    run: ExecFileRunner = execFileAsync,
  ) {
    this.log = log;
  ...
  async isAvailable(): Promise<boolean> {
    ...
    try {
      const { stdout } = await this.run(this.executable, ['--version'], { timeout: 5_000 });
  ...
  async exec<T extends WeComBaseResponse>(
    ...
    try {
      const { stdout, stderr } = await this.run(this.executable, args, {
