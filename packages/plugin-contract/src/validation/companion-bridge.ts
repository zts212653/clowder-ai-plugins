...
function audioSdp(sdp: string): boolean {
  return sdp.startsWith('v=0') && /^m=audio /m.test(sdp) && !/^m=video /m.test(sdp);
}
...
export function validateCompanionCommand(value: unknown): value is CompanionCommand {
  ...
  if (parsed.kind === 'offer') return audioSdp(parsed.sdp);
...
export function validateCompanionReply(value: unknown): value is CompanionReply {
  ...
  return parsed.kind !== 'answer' || audioSdp(parsed.sdp);
}
