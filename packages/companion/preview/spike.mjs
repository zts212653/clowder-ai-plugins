...
function render() {
  ...
  $('begin').hidden = active;
  $('begin').textContent = s.call === 'failed' ? '重试语音' : '语音聊';
  ...
  for (const id of ['share', 'menu-share']) $(id).textContent = s.sharing ? '停止共享' : '共享画面';
  $('documents').textContent = `资料查询 · ${s.documents ? '开启' : '暂停'}`;
