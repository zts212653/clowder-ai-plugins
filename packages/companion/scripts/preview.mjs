...
<header><h1>猫猫球 · 外观核对</h1><p>同一份正式 renderer 构建：透明猫身、小“聊聊”按钮、通话语音条，文字按需展开。<br><small>这里仅模拟连接状态，不连接家里、不采集麦克风或屏幕；尚未替换已安装版本。</small></p>
...
<label>情境 <select id="scenario"><option value="normal">正常连接</option><option value="failure">连接失败</option></select></label><button id="background">切换深色桌面</button><p id="note" role="status"></p></header>
<main><iframe title="实际猫猫球界面" src="/index.html?skin=ragdoll-v1"></iframe></main>
<script>const frame=document.querySelector('iframe');function refresh(){frame.style.width='300px';frame.style.height='270px';frame.src='/index.html?skin='+document.querySelector('#skin').value+'&scenario='+document.querySelector('#scenario').value;}document.querySelectorAll('select').forEach(e=>e.onchange=refresh);document.querySelector('#background').onclick=()=>document.querySelector('main').classList.toggle('dark');window.addEventListener('message',e=>{if(e.source!==frame.contentWindow||e.origin!==location.origin)return;if(e.data.kind==='resize'){frame.style.width=e.data.expanded?'380px':'300px';frame.style.height=e.data.expanded?'590px':'270px';}if(e.data.kind==='note')document.querySelector('#note').textContent=e.data.text;});</script></html>`;

function fixture(skin, failure) {
  return `<script>
  const listeners = new Set(); let phase = 'idle', allowed = true;
  ...
  window.clowderCompanion = {subscribe(fn){listeners.add(fn);return()=>listeners.delete(fn);},async request(command){
    switch(command.kind){
      ...
      case 'audio.connect': setTimeout(()=>{if(${failure}){emit({kind:'audio',type:'error',code:'carrier_unavailable'});}else{phase='talking';emit({kind:'audio',type:'connected'});}},450);return {kind:'ok'};
      ...
      case 'text':emit({kind:'audio',type:'transcript',role:'assistant',text:'这是一条外观预览文字，没有发送给模型。'});return {kind:'delivery',delivery:'accepted'};
...
}).listen(3891, '127.0.0.1', () => console.log('Companion visual preview: http://127.0.0.1:3891 (no Host or media)'));
