...
function multiBindingHost(bindings: Array<{ key: string; threadId: string }>, warnings: unknown[][]) {
  return {
    config: { get: async key => ({ appId: 'app', connectionMode: 'webhook' })[key] },
    secrets: { get: async key => ({ appSecret: 'secret', verificationToken: 'token' })[key] },
...
function startModule(outbound: FeishuAdapter, bindings: Array<{ key: string; threadId: string }>, warnings: unknown[][]) {
...
test('multi-binding outbound delivers to the healthy chat when one binding send fails', async () => {
  ...
  await active.actions['feishu.outbound']?.(delivery());
