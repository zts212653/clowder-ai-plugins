...
- **update-holding 到 uninstall snapshot 的 lineage 收口**：Plugin Manager 在卸载 disposition census 中既枚举当前 attached stores/datasets，也枚举 **same pluginInstanceId** 的全部 standalone update-holding entries；绝不吸收其他 pluginInstanceId 已完成的 uninstall snapshots。Manager 先按每个旧 entry 的原 policy 处置：`lifecycle` 删除，`retained` 保留，`ask-on-uninstall` 纳入本次用户选择。若仍有任一 store/dataset 需保留，整个 uninstall 只签发一个 `bundleKind=uninstall-snapshot` 的新 `detachedBundleId A`，在同一 durable commit 中把本次 attached 保留内容写入 A、把仍非空的 U 加入 `absorbedDetachedBundleIds`、并在每个 U 写 `absorbedByDetachedBundleId=A`；entry 的原 key、首次 `sourceOperation`、source packageRevision、dataClass/policy/schemaVersion/contentDigest 保持不变。成功后 same pluginInstanceId 不得残留 standalone U；U 只作为 A 的 immutable child 在 Settings 嵌套展示和精确 clear。失败/crash 只能收敛到“installed + 原 standalone U、无 A”或“absent + 完整 A closure、无 standalone U”，重试必须复用 journal operation/bundle ID，不能重复生成 A、丢 entry、形成环/多父边或 split lineage。
- reinstall 始终创建 **fresh pluginInstanceId** 及全新的 lease、cursor 与幂等/结算账本空间，绝不复活旧 runtime authority。只有新 package 的 `pluginId + publisher identity + origin` 与 detached record/inventory 完全匹配，且用户显式选择 **恰好一个 `bundleKind=uninstall-snapshot` 的 `detachedBundleId` generation** 恢复时，Host 才能把 A 的 direct records 和 Host-declared `absorbedDetachedBundleIds` closure 中实际存在的旧 config/secrets/state 与 datasets 作为同一 restore input；standalone update-holding 不作为 reinstall input。禁止用户跨两个 top-level snapshots 选择、临时拼 arbitrary bundle-set 或用“最新一代”隐式替自己决定。dataset 的 stable ID/dataClass/policy 必须兼容，schemaVersion 变化必须提供声明式 migration。Host 在创建 migration staging 前必须先按 stable datasetId 枚举 A closure 的完整候选集：A direct 与任意 child U 中实际存在的 entry 都是候选，精确键统一为 `(entry.detachedBundleId, datasetId)`，因此 direct 项为 `A + X`、child 项为 `U + X`；Settings 从 committed inventory/lineage 与待安装的 verified manifest/migration plan **纯投影**每项的 compatibility/migration eligibility 与原 provenance，不另存一份会漂移的 candidate list。当同一 ID 的原始候选多于一项时，用户必须在 eligible candidates 中显式选择至多一个 exact candidate；未选择时该 ID 默认不恢复，Host 不得猜“当前”“最新”或偏向 direct/child。restore journal 在 staging 前冻结所选 A、inventory revision、verified package revision 与 exact candidate keys，Plugin Manager 在同一 transaction 中重新验证每个 key 仍属于 A closure、仍 eligible 且同 ID 未多选；restore 与 concurrent clear 串行，stale/foreign/ineligible/duplicate selection 或 package revision 漂移一律在消费任何 entry 前 fail closed，crash retry 复用 journal 中同一选择。只有选中的 candidate 才进入 migration staging，未选或不兼容的 entry 保持原 key/lineage detached；单一 candidate 仍走普通 compatibility/migration 路径。Host 在 package/config/state/dataset migration 全部成功后才把选中 logical generation 的可恢复内容原子绑定给新实例并移除已绑定条目；每当 restore 或 clear 排空 child U，必须同批移除 U metadata、A 的对应 `absorbedDetachedBundleIds` 成员与 reciprocal pointer，A 的 direct records 和 child list 全空后才移除 A metadata。`lifecycle` 与新增 dataset 从空内容开始，未在新 manifest 存续或未被选择的 dataset 仍在原 A closure 保持 detached，其他未选 top-level generations 全部原样保留。不同 signer/origin 不得看见或认领旧数据。恢复失败时选中 logical generation 的完整 closure 原样保留，新实例保持未配置、disabled，不能出现跨代混合、部分 dataset 可读、半绑定或旧新实例双读；成功激活后，fresh context 必须能按 grant 读取恢复后的 dataset，旧 context 继续 fail closed。
...
|---|---|---|---|
| bundle kind、source instance、absorption lineage | Plugin Manager durable operation journal | recovery、audit、Settings | inventory 只从已提交 journal 投影；retry 复用 operation/bundle ID |
...
| entry 当前归属（attached/standalone U/A direct/A child/deleted） | Host attached/detached inventory + Host lineage edges | runtime authorization、Settings list/clear、reinstall staging | journal commit 原子切换 logical membership；禁止 standalone U 同时属于 A 或多父 A |
| reinstall 可恢复集合 | 用户选择的单一 uninstall snapshot + verified manifest | Plugin Manager restore transaction | 仅 compatible/migrated entries 派生为 fresh-instance bindings；其余仍在原 A |
...
| installed + 零或多个 same-instance standalone U | uninstall | 原 policy 删除/询问/保留；当前 stores/datasets 直接写唯一 A，非空 U 原子 link 为 A child | installed + 原 standalone U、无 A |
| uninstall commit 已落盘 | restart/retry | absent + 完整 A closure；journal 重放同一 A 与同一 `absorbedDetachedBundleIds` | 不得把 U 改回 standalone、创建 A2、环、多父边或重复 entry |
| absent + A closure，child U 非空 | clear `U + datasetId` | 只删 exact entry；U 排空则原子删 reciprocal edge/U metadata；closure 全空才删 A | A closure 与 entry 原样保留，无 dangling edge |
| absent + A closure，同 ID 存在 A direct/child U 多候选 | 用户选择恢复 A 并提交 exact candidate keys | restore journal 冻结 A/inventory/package revision/keys；同 ID 在完整候选集中显式至多选一，仅选中且复验 eligible 的 entry 进入 staging；未选项仍留 A | stale/foreign/ineligible/duplicate key、package revision 漂移或任一失败均使 A closure 原样保留；新实例未配置、disabled |
| absent + 多个不同 pluginInstanceId 的 uninstall snapshots A/B | 用户选择恢复 A | fresh instance 只枚举 A closure 并按上一行处理 collision；兼容/迁移成功项绑定，排空 child 时清边，未选/未接纳项仍留 A，B 不变 | A/B closures 全部原样保留；新实例未配置、disabled |

可测试不变量：

- **INV-DL1（单一逻辑归属）**：每个 store/dataset entry 恰好处于 attached、standalone U、一个 A 的 direct/child closure 或 deleted 之一；U 不得 standalone 且有 absorbedBy，也不得多父；inventory/graph uniqueness 断言。
- **INV-DL2（同实例收口）**：成功 uninstall 后，same pluginInstanceId 的所有非空 U 都只作为唯一 A child，所有需保留内容只由 A logical generation 可选；update→uninstall journey 断言。
- **INV-DL3（跨实例隔离）**：收口不得修改其他 pluginInstanceId 的历史 uninstall snapshots；A/B generation journey 断言。
- **INV-DL4（单 bundle 恢复）**：reinstall 只接受用户选择的一个 uninstall snapshot，拒绝 update-holding 与 bundle-set；negative restore fixture 断言。
- **INV-DL5（crash 原子性）**：任一 crash point 只可见 installed + 完整 standalone U 或 absent + 完整 A closure，且 journal retry 不生成第二个 A/lineage edge；fault-injection matrix 断言。
...
- **INV-DL7（图引用完整性）**：每个 A→U 都有 U→A reciprocal pointer，U 只能一个父且非空；clear/restore 排空 U 时同批删双向边，logical closure 全空才删 A；graph referential-integrity/fault-injection 断言。
- **INV-DL8（同 ID 候选完备）**：A closure 中某 stable datasetId 的原始候选全集恰好覆盖 A direct 与每个 child U 的 entry；多候选时 selector 以 `(entry.detachedBundleId, datasetId)` 唯一标识 eligible candidate，至多选一且零选择不恢复，只有选中项进入 staging，未选项保持原 key/lineage detached；direct-vs-U 与 U-vs-U collision journey 断言。
...
| 行为 | 必须保持 | 保护方式 |
|---|---|---|
| 两个已完成 uninstall generations A/B | 互不拼接、互不改写，用户只选一个 | 既有 A/B reinstall journey + INV-DL3/DL4 |
...
| A direct/child 同 ID | 不偏向 direct/child；用户可精确选择任一 eligible candidate，未选则都不恢复；stale/越界选择不消费数据 | direct-vs-U collision journey + INV-DL8/DL9 |
