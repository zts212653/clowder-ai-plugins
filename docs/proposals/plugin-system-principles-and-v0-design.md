...
- reinstall 始终创建 **fresh pluginInstanceId** 及全新的 lease、cursor 与幂等/结算账本空间，绝不复活旧 runtime authority。只有新 package 的 `pluginId + publisher identity + origin` 与 detached record 完全匹配且用户显式选择恢复时，Host 才能把旧 config/secrets/state 作为 staging 输入，按新 schema migration 后原子绑定给新实例；不同 signer/origin 不得看见或认领旧数据。恢复失败时 detached record 原样保留，新实例保持未配置、disabled，不能出现半绑定或旧新实例双读。
- explicit clear 是 Host 控制面的用户动作，可分别针对 config、secrets、namespaced state，已安装与 detached 两种记录都必须支持。对已安装实例，Host 先撤销相关 lease、停止 delivery、在 journal 中原子清除所选 store，再用新 activation revision reconcile；clear config 使 readiness 回到 `incomplete`，clear secrets 立即撤销对应凭据访问，clear state 删除完整 namespace/checkpoint。失败或 crash 必须回滚到清除前 snapshot；clear 不删除审计/transaction ledger，也不能由插件自行调用或伪装成 uninstall/repair/update 的副作用。
