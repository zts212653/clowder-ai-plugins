...
- `@clowder-ai/plugin-sdk`：提供 schema-neutral standalone stdio runtime、握手/`events.publish` Host-bound helper 与 carrier-neutral package-module facade。声明 `runtime.transport: builtin` + root `entrypoint` 的包必须让该 ESM 模块的 `default` export 通过 `requirePluginModuleEntrypoint(...)`，Host 不猜测 named exports。
